...
public class MapSelection {
    ...
    public void draw(GraphicsContext g){
        ...
        for(MapSelectionItem item : mapSelectionItems){
            ...
            item.setDimension( new Dimension2D(width, height));
            item.setLocation( new Point2D((1280-width)/2, item.getLocation().y));
    ...
    private MapSelectionItem[] createItems(ArrayList<String> maps){
        ...
        for (int i = 0; i < items.length; i++) {
            Point2D location = new Point2D(0, (i+1)*defaultGridSize+200);
    ...
    public String selectMap(Point2D mouseLocation) {
        for(MapSelectionItem item : mapSelectionItems) {
            Dimension2D dimension = item.getDimension();
            Point2D location = item.getLocation();
