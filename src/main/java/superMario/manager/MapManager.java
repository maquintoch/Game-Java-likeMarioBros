...
public class MapManager {
    ...
    private void checkBottomCollisions(GameEngine engine) {
        ...
        if (mario.getY() + mario.getDimension().getHeight() >= map.getBottomBorder()) {
            mario.setY(map.getBottomBorder() - mario.getDimension().getHeight());
            mario.setFalling(false);
            mario.setVelY(0);
        }
    ...
    private void checkMarioHorizontalCollision(GameEngine engine){
        ...
        for (Brick brick : bricks) {
            ...
            if (marioBounds.intersects(brickBounds)) {
                mario.setVelX(0);
                if(toRight)
                    mario.setX(brick.getX() - mario.getDimension().getWidth());
                else
                    mario.setX(brick.getX() + brick.getDimension().getWidth());
            }
        }
        ...
        for (Enemy enemy : enemies) {
            ...
            for (Brick brick : bricks) {
                ...
                if (enemyBottomBounds.intersects(brickTopBounds)){
                    ...
                    enemy.setVelY(0);
                    enemy.setY(brick.getY()-enemy.getDimension().getHeight());
                    standsOnBrick = true;
                }
            }

            if(enemy.getY() + enemy.getDimension().getHeight() > map.getBottomBorder()){
                enemy.setFalling(false);
                enemy.setVelY(0);
                enemy.setY(map.getBottomBorder()-enemy.getDimension().getHeight());
            }
    ...
    private void checkPrizeCollision() {
        ...
        for (Prize prize : prizes) {
            if (prize instanceof BoostItem) {
                ...
                for (Brick brick : bricks) {
                    ...
                    if (boost.isFalling()) {
                        ...
                        if (brickBounds.intersects(prizeBottomBounds)) {
                            ...
                            boost.setVelY(0);
                            boost.setY(brick.getY() - boost.getDimension().getHeight() + 1);
                            if (boost.getVelX() == 0)
                                boost.setVelX(2);
                        }
                    }
                ...
                if (boost.getY() + boost.getDimension().getHeight() > map.getBottomBorder()) {
                    boost.setFalling(false);
                    boost.setVelY(0);
                    boost.setY(map.getBottomBorder() - boost.getDimension().getHeight());
                    if (boost.getVelX() == 0)
                        boost.setVelX(2);
                }
    ...
    private void removeObjects(ArrayList<Afigures> list){
        if(list == null)
            return;

        for(Afigures object : list){
