...
import superMario.figures.brick.*;
import superMario.figures.prize.*;
import superMario.view.ImageLoader;
import superMario.maps.Map;
import superMario.figures.enemy.Enemy;
import superMario.figures.enemy.Goomba;
import superMario.figures.enemy.KoopaTroopa;
...
class MapCreator {
    ...
    private Image superMushroom, oneUpMushroom, fireFlower, coin;
    private Image ordinaryBrick, surpriseBrick, groundBrick, pipe;
    private Image goombaLeft, goombaRight, koopaLeft, koopaRight, endFlag;


    MapCreator(ImageLoader imageLoader) {

        this.imageLoader = imageLoader;
        Image sprite = imageLoader.loadImage("/sprite.png");
    ...
    Map createMap(String mapPath, double timeLimit) {
    	Image mapImage = imageLoader.loadImage(mapPath);
        ...
        // need to check if hashcode works, or toString?
        int mario = Color.rgb(160, 160, 160).hashCode();
        int ordinaryBrick = Color.rgb(0, 0, 255).hashCode();
        int surpriseBrick = Color.rgb(255, 255, 0).hashCode();
        int groundBrick = Color.rgb(255, 0, 0).hashCode();
        int pipe = Color.rgb(0, 255, 0).hashCode();
        int goomba = Color.rgb(0, 255, 255).hashCode();
        int koopa = Color.rgb(255, 0, 255).hashCode();
        int end = Color.rgb(160, 0, 160).hashCode();

        for (int x = 0; x < mapImage.getWidth(); x++) {
            for (int y = 0; y < mapImage.getHeight(); y++) {

                int currentPixel = mapImage.getPixelReader().getArgb(x, y);
